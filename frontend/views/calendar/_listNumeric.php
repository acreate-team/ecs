...
<div id="content" class="calendar_list calendar_list-alphabet">
	<div class="calendar_listWrap">
		<div style="display: table; margin: auto; width: 750px;">
			<table>
				<tr class="head">
				...
				<?php foreach ($page as $key => $p) : ?>
					...
					<?php endif; ?>
						...
						<?php $numeric = sprintf("%03d", $num); ?>

						<?php if(isset($match) && $p->match) : ?>
						<td style="text-align: center; padding-left: 5px" class="match-q">
							<a href="/system/ks-<?=$p->url?>" data-id="<?=$p->id?>" data-numeric="<?=$p->numeric_calendar?>" style="font-size: 18px">
								<?=$numeric?>
							</a>
						</td>
						<?php else: ?>
